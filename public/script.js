  ...
  if (targetLink) targetLink.classList.add("active");
     ...
     if (contactForm) {
      ...
      chatButton.addEventListener("click", function () {
          ...
          chatButton.classList.toggle("active"); // Rotate button when open
      });
  
      // Close chatbot
      closeChat.addEventListener("click", function () {
          chatbotContainer.classList.remove("active");
          chatButton.classList.remove("active"); // Reset button state
      });
      ...
      function sendChatMessage() {
          ...
          // Auto-reply (can be replaced with actual chatbot logic)
          setTimeout(() => {
              const botMessage = document.createElement("div");
              botMessage.classList.add("message", "bot");
              botMessage.textContent = "I'm here to assist you!";
              chatResponse.appendChild(botMessage);
              chatResponse.scrollTop = chatResponse.scrollHeight; // Scroll to latest message
          }, 500);
     ...
     async function sendChatbotMessage() {
      ...
      try {
          ...
          chatResponse.innerHTML = `<p>${data.reply}</p>`;
      } catch (error) {
          chatResponse.innerHTML = "Error connecting to chatbot.";
      }
  }
